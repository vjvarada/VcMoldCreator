...
export function classifyMoldHalves(
  ...
  const coveredCount = h1Triangles.size + h2Triangles.size + boundaryZoneTriangles.size + innerBoundaryTriangles.size;
  if (coveredCount !== triangles.length) {
    console.warn(`Mold Half Classification: Coverage mismatch! Covered ${coveredCount} but total is ${triangles.length}`);
    // Find uncovered triangles
    const allCovered = new Set<number>();
    h1Triangles.forEach(i => allCovered.add(i));
    h2Triangles.forEach(i => allCovered.add(i));
    boundaryZoneTriangles.forEach(i => allCovered.add(i));
    innerBoundaryTriangles.forEach(i => allCovered.add(i));
    ...
    for (let i = 0; i < triangles.length; i++) {
      if (!allCovered.has(i)) {
        ...
        if (uncoveredCount <= 10) {
          console.warn(`    Uncovered triangle: ${i}`);
        }
      }
    }
    if (uncoveredCount > 10) {
      console.warn(`    ... and ${uncoveredCount - 10} more uncovered triangles`);
    }
  }
