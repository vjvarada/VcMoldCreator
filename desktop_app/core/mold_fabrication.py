...
def create_metamold_clamp(
    ...
    # ------------------------------------------------------------------
    metamold_silhouette_2d = prism_result.silhouette_2d  # already offset by wall+margin
    ...
    # slit box in 2D: along slit_edge_unit by slit_half_w each side of midpoint,
    # along slit_outward_normal by slit_depth from inner to beyond outer
    ...
    #     Each tab: width = clamp_height (full height extrusion), length = tab_length
    tab_thickness = wall_thickness  # same as clamp wall
    tab_half_t = tab_thickness / 2.0
    tab_meshes = []
    for sign in (-1, 1):
        # Tab centre sits at the slit edge, offset by slit_half_w + tab_half_t
        tab_anchor_2d = slit_midpoint_2d + slit_edge_unit * sign * (slit_half_w + tab_half_t)
        # Tab extends outward from the clamp wall
...
def build_metamold_halves_manifold_space(
    ...
    try:
        ...
        elapsed_ms = (time.time() - start_time) * 1000
        logger.exception("[manifold-space] Failed: %s", exc)
...
def split_shell_with_membrane(
    ...
    try:
        ...
        elapsed_ms = (time.time() - start_time) * 1000
        logger.exception("Failed to split shell with membrane: %s", e)
        return None, None, None, elapsed_ms, False
...
def thicken_surface_symmetric(
    ...
    try:
        ...
        elapsed_ms = (time.time() - start_time) * 1000
        logger.exception("Failed to thicken surface: %s", e)
        return None, elapsed_ms, False
...
def create_part_with_thickened_secondary(
    ...
    try:
        ...
        elapsed_ms = (time.time() - start_time) * 1000
        logger.exception("Failed to create part with thickened secondary: %s", e)
        return None, elapsed_ms, False
