...
def find_secondary_cutting_edges(
    ...
    # Include ALL same-label edges, including surface-to-surface edges
    # Surface edges can still have divergent escape paths around part features
    candidate_edges = []
    n_on_part_surface = 0
    n_surface_to_surface = 0
    ...
    for e_idx in np.where(both_interior)[0]:
        ...
        if label0 == label1 and label0 > 0:
            # Track surface vertex counts for logging
            v0_on_surface = v0 in part_surface_vertices
            v1_on_surface = v1 in part_surface_vertices
            
            # Include ALL edges, including surface-to-surface
            candidate_edges.append((v0, v1, idx0, idx1))
            
            if v0_on_surface and v1_on_surface:
                n_surface_to_surface += 1
            elif v0_on_surface or v1_on_surface:
                n_on_part_surface += 1
    
    timing_log['phase3_candidate_edges'] = (time.time() - phase_start) * 1000
    logger.info(f"Phase 3 (candidate edges): {timing_log['phase3_candidate_edges']:.1f}ms - {len(candidate_edges)} edges ({n_surface_to_surface} surface-to-surface, {n_on_part_surface} surface-to-interior)")
